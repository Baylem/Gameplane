...
**Binding Modules**: `web/src/lib/theme-export.ts`, `web/src/components/ui/ThemeSettingsModal.tsx`, `web/src/lib/theme-sanitize.ts`  
**Status**: Binding  
...
2. Offered in the Theme Settings modal as:
...
1. User pastes JSON text or selects a `.json` file in the Theme Settings modal.
...
4. The server re-validates everything and applies FR-013 sanitization to `customCss`; a `400` response is surfaced in the modal with the server message identifying the offending rule.
