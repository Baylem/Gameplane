...
- [ ] T002 [P] Confirm the Pencil MCP server is available and `design.pen` opens, since Constitution Principle II requires design-first work in design.pen before any frontend code (blocks T003)
...
- [ ] T003 Design the Theme & Appearance surfaces in design.pen via the Pencil MCP server per contracts/theme-ui.md — ThemeSettingsModal (Presets / Custom Colors / Custom CSS / Export tabs), TopBar and Sidebar triggers, SafeModeBanner, and the login-page "Sign in with safe mode" link — then re-export every touched node to design-export/json/ and design-export/screenshots/ (Constitution II)
...
- [ ] T018 [P] [US1] Create the ThemeSettingsModal shell and Presets tab in web/src/components/ui/ThemeSettingsModal.tsx per contracts/theme-ui.md §2/§3.1 and the T003 design: HeroUI Modal + Tabs, preset radio cards with swatches, Light/Dark/System segmented control, retention note, footer with Reset to Defaults placeholder (wired in T040)
- [ ] T019 [P] [US1] Add the "Theme & Appearance" item (Palette icon) to the user avatar dropdown in web/src/components/ui/TopBar.tsx per contracts/theme-ui.md §1.1
- [ ] T020 [P] [US1] Add the customize-theme icon button (aria-label="Customize theme") to the appearance footer in web/src/components/ui/Sidebar.tsx per contracts/theme-ui.md §1.2
- [ ] T021 [US1] Write modal unit tests in web/src/components/ui/ThemeSettingsModal.test.tsx: renders tabs, clicking "Legacy" sets data-theme-preset="legacy", save calls PUT /users/me/preferences via useThemePreferences, switching presets does not clear stored custom values (depends on T018)
...
**Independent Test**: Choose a custom accent + surface in the Custom Colors tab, confirm the palette renders across navigation/buttons/panels, toggle light/dark and confirm legibility, then revert to a preset (spec.md US3)
...
- **US3 (P2)**: Starts after Foundational — reuses the modal shell from US1 (T018) and appends to the same Playwright spec file
- **US4 (P3)**: Starts after Foundational — reuses the modal shell (T018) and appends to the same Playwright spec file; server-side sanitization already landed in Foundational T007

### Within Each User Story

- Tests are written first (or alongside) and must fail before implementation makes them pass
- Library utilities (theme-derivation, theme-sanitize, theme-export) before the modal tabs that consume them
...
- web/src/components/ui/ThemeSettingsModal.tsx: T018 → T021/T030 → T036 → T037 → T040
...
Task: "Create ThemeSettingsModal shell in web/src/components/ui/ThemeSettingsModal.tsx"
