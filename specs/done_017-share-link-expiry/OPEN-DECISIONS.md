...
- **OD-8 — Settled 2026-09-20:** `web/src/types.ts`'s `expiresAt` widening and the MSW mock changes ship with Phase 5 (UI), not with the API slice, because `ShareLinks.tsx`'s `formatDate`/`getLinkStatus` would need null handling, which is UI work blocked by design-first (CLAUDE.md rule 1). Until then the UI cannot create a never-expiring link, so the API never returns null to it.
- **OD-9 — Settled 2026-09-20 (maintainer, in chat):** the create dialog's four conditional expiry states are drawn as SEPARATE frames rather than stacked into one, following the OD-23 precedent from feature 014 that split `J5pjJ3`'s five address states and the `QQtUD` "as built" frame: `atqRh` stays canonical (the "30 days" preset selected, no warning, no date picker), and two new sibling frames depict the "No expiry" state (with "This link works until you revoke it.") and the "Custom" state (date picker plus the 365-day long-lived warning). Drawing all four into one frame is rejected because it would depict a state the app can never render — the same defect class as the impossible `f1Vga` alert in issue #376. The custom date control is drawn on the existing `Gameplane/Input` master `D0cDM` (36px tall, 12px corners, field tokens per feature 014's OD-20) with a trailing calendar icon, matching what HeroUI's DateInput actually renders; no new shared master is created, and reusing the Select master `AT7ya` is rejected because the app renders a date input, so that design could never converge against the visual-diff gate.
