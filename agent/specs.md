...
- **Partial uploads never linger**: `files.savePart` removes the destination file it just created whenever a save fails after the file was opened — a source read error, an `io.ErrUnexpectedEOF` from a truncated multipart body, or an over-the-limit part. Without this a client abort mid-upload could leave a half-written file that a later `/files/read` or `/files/download` serves as though it were complete.
...
- **No half-written uploads**: `files.savePart` deletes the destination file it opened whenever a save fails partway through (truncated body, read error, over-limit part), so a failed upload can never leave a partial file that a subsequent read or download would serve as if it were whole.
